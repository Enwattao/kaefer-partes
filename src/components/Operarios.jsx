...
import { getOperarios, saveOperarios, genId } from '../lib/db.js'
...
export default function Operarios() {
  ...
  useEffect(() => { getOperarios().then(setItems) }, [])

  async function persist(next) { setItems(next); await saveOperarios(next) }

  function add() {
    const val = nuevo.trim()
    if (!val) return
    if (items.some(i => i.nombre.toLowerCase() === val.toLowerCase())) { setError('Ya existe ese operario'); return }
      ...
      <div className="page-pad" style={{ display: 'flex', flexDirection: 'column', gap: 18, maxWidth: 720, width: '100%', margin: '0 auto' }}>
          ...
          <div className="card-body" style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            <div style={{ flex: 1 }}>
              <input
                ...
                placeholder="Nombre y apellidos del operario…"
                value={nuevo}
        ...
        <div className="card">
          <div className="card-header" style={{ justifyContent: 'space-between' }}>
            <span style={{ fontWeight: 700 }}>Lista de operarios</span>
            <span className="chip chip-gray">{items.length}</span>
          </div>
          {items.length === 0 ? (
            <div className="empty"><div className="empty-icon">👷</div><div className="empty-text">Sin operarios todavía</div></div>
          ) : items.map(item => (
